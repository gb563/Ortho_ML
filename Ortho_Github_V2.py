...
def process_ortho_data(cpt_code):
    ...
    cols4 = ['CPT','OTHERCPT1','ELECTSURG','Age','SEX','FNSTATUS2','WNDINF',
             'EMERGNCY','PRSEPIS','DIABETES','DYSPNEA','ASACLAS','STEROID',
             'ASCITES','VENTILAT','DISCANCR','HYPERMED','HXCHF','SMOKE','HXCOPD',
             'DIALYSIS','RENAFAIL','HEIGHT','WEIGHT','DISCHDEST']
    
    #load all the datasets from disk (need to request NSQIP datasets first)
    df11 = pd.read_spss(r'C:\File_path.sav', usecols=cols4)
    df12 = pd.read_spss(r'C:\File_path.sav', usecols=cols3)
    df12.rename(columns={"AGE": "Age"},inplace=True)
    df13 = pd.read_spss(r'C:\File_path.sav', usecols=cols4)
    df14 = pd.read_spss(r'C:\File_path.sav', usecols=cols4)
    df15 = pd.read_spss(r'C:\File_path.sav', usecols=cols3)
    df15.rename(columns={"AGE": "Age"},inplace=True)
    df16 = pd.read_spss(r'C:\File_path', usecols=cols3)
    df16.rename(columns={"AGE": "Age"},inplace=True)
    df17 = pd.read_spss(r'C:\File_path.sav', usecols=cols3)
    df17.rename(columns={"AGE": "Age"},inplace=True)
    df18 = pd.read_spss(r'C:\File_path.sav', usecols=cols3)
    df18.rename(columns={"AGE": "Age"},inplace=True)
    
    #combine each year into one large dataframe
    data2=pd.concat([df11,df12,df13,df14,df15,df16,df17,df18],axis=0)
    data2 = shuffle(data2)
    ...
    x1=data2['SEX']
    x1=x1.replace({'NULL':np.nan,'male':0,'female':1})
    ...
    return inputs_NSQIP, inputs_cal, inputs_test, targets_train, targets_cal, targets_test



def demographics(inputs_NSQIP,inputs_test):
    ...
    #dem_pd.to_excel(r'C:\Users\---your file path and file name.xlsx', index = False)
    print(dem_pd)
...
def optimized_data(inputs_NSQIP,targets_train,rand_points=20,search_number=100):
...
def bootstrap_internal(inputs_NSQIP,targets_train,optimized_params):
    ...
    print('Brier Score = ',av_brier,', 95% CI = ',brier_hold[25],'to ',brier_hold[975])
    


#now we need to fit the final model (above was just estimating performance)
#and then perform calibration
def calibrate_model(inputs_NSQIP,targets_train,inputs_cal,targets_cal,inputs_test,targets_test,optimized_params):
    ...
    OrthoV1.fit(inputs_NSQIP,targets_train.ravel(), eval_metric = 'logloss')
    ...
    #calibrate model sigmoid
    cal_sig = CalibratedClassifierCV(OrthoV1, method='sigmoid', cv='prefit')
    cal_sig.fit(inputs_cal, targets_cal.ravel())
    
    #make some predictions!
    probs = OrthoV1.predict_proba(inputs_test)
    probs = probs[:,1]
    probs2 = cal_iso.predict_proba(inputs_test)
    probs2=probs2[:,1]
    probs3 = cal_sig.predict_proba(inputs_test)
    probs3=probs3[:,1]
    
    #brier for all three
    b_loss = brier_score_loss(targets_test,probs)
    b_loss_iso = brier_score_loss(targets_test,probs2)
    b_loss_sig = brier_score_loss(targets_test,probs3)
    
    #calibration curves for all three
    fop, mpv = calibration_curve(targets_test,probs,n_bins = 10,strategy='quantile')
    fop2, mpv2 = calibration_curve(targets_test,probs2,n_bins = 10)
    fop3, mpv3 = calibration_curve(targets_test,probs3,n_bins = 10)
    ...
    ax.plot(mpv3,fop3,'g',label='Sigmoid, Brier = {:.3f}'.format(b_loss_sig))
    ax.plot([0,1],[0,1],'k--',label='Perfect Calibration')
    ax.legend(loc = 'lower right')
    ax.plot(mpv,fop,'b.')
    ax.plot(mpv2,fop2,'r.')
    ax.plot(mpv3,fop3,'g.')
    ax.set_title('Calibration Curves for Holdout Data')
...
def bootstrap_test(inputs_NSQIP,targets_train,inputs_test,targets_test,optimized_params):
    ...
    OrthoV1.fit(inputs_NSQIP,targets_train.ravel(), eval_metric = 'logloss')
    preds= OrthoV1.predict_proba(inputs_test)
    preds=preds[:,1]
...
def clinical_impact(inputs_NSQIP,targets_train,inputs_test,targets_test,optimized_params):
    
    preds = create_model(inputs_NSQIP,targets_train,inputs_test,optimized_params)
    Thresholds = np.linspace(0.1, .8, 8, endpoint=True)
    sens_XGB = []
    spec_XGB = []
    ppv_XGB=[]
    num_tp = []
    ...
    multiplier = (targets_test==1).sum()/(len(targets_test))
    prev = 1000*multiplier
    tp1000 = np.around(prev*np.array(sens_XGB))
    fn1000=np.round(prev*(1-np.array(sens_XGB)))
    fp1000=np.round((1000-prev)*(1-np.array(spec_XGB)))
    tn1000=np.round((1000-prev)*np.array(spec_XGB))
    tt2=np.concatenate([100*Thresholds.reshape(-1,1),sens_XGB,spec_XGB,ppv_XGB,fn1000,tp1000,fp1000,tn1000],axis=1)
    tt2_pd=pd.DataFrame(tt2)
    tt2_pd.columns=['Threshold','Sensitivity (Recall)','Specificity','PPV (Precision)','False Negatives','True Positives','False Positives','True Negatives']
    #tt2_pd.to_excel(r'C:\Users-----enter your path and file name.xlsx', index = False)
    print(tt2_pd)



def shap_plots(inputs_NSQIP, inputs_test, targets_train, targets_test,optimized_params):
    ...
    #summary plot
    names2=['Age','Sex','Fxn','ASA PS','Steroids','Ascites','SIRS','Ventilator','Cancer','Diabetes','HTN','CHF','Dyspnea','Smoker','COPD','Dialysis','Renal Failure','BMI']
    explainer=shap.TreeExplainer(OrthoV1)
    shap_values=explainer.shap_values(inputs_test)
    shap.summary_plot(shap_values,inputs_test,feature_names=names2,show=True)
    ...
    sens_false_neg_hold = []
    least_conf=[]
    probs=create_model(inputs_NSQIP,targets_train,inputs_test,optimized_params)
    preds_hold = np.concatenate([probs.reshape(-1,1),targets_test],axis=1)
    ...
    data_true_pos = inputs_test[sens_true_pos_hold]
    
    sens_false_neg_hold=array(sens_false_neg_hold)
    data_false_neg = inputs_test[sens_false_neg_hold]
    
    #plot all of the force_plots
    #true positives
    #basic formatting for display purposes only
    inputs_test2=inputs_test.copy()
    
    for i in range(len(inputs_test2)):
        inputs_test2[i,-1]=round(inputs_test2[i,-1],1)
    
    for i in range(0,5):
        shap_display=shap.force_plot(explainer.expected_value,shap_values[sens_true_pos_hold[i],:],inputs_test2[sens_true_pos_hold[i],:],matplotlib=True,feature_names=names2,show=True)
        print(preds_hold[sens_true_pos_hold[i],0],preds_hold[sens_true_pos_hold[i],1])
        #plt.savefig(('SHAP_force_plot_TP_plot_'+str(i)+'.jpeg'), edgecolor='black', dpi=1200,bbox_inches="tight")
        
    #false negatives
    for i in range(0,5):
        shap_display=shap.force_plot(explainer.expected_value,shap_values[sens_false_neg_hold[i],:],inputs_test2[sens_false_neg_hold[i],:],matplotlib=True,feature_names=names2,show=True)
        print(preds_hold[sens_false_neg_hold[i],0],preds_hold[sens_false_neg_hold[i],1])
        #plt.savefig(('SHAP_force_plot_FN_plot_'+str(i)+'.jpeg'), edgecolor='black', dpi=1200,bbox_inches="tight")
        
    #least confident
    for i in range(0,5):
        shap_display=shap.force_plot(explainer.expected_value,shap_values[least_conf[i],:],inputs_test2[least_conf[i],:],matplotlib=True,feature_names=names2,show=True)
        print(preds_hold[least_conf[i],0],preds_hold[least_conf[i],1])
        #plt.savefig(('SHAP_force_plot_Low_prob_plot_'+str(i)+'.jpeg'), edgecolor='black', dpi=1200,bbox_inches="tight")
       

    #SHAP dependency plots
    df_inputs_test=pd.DataFrame(inputs_test, columns=names2)
    df_knees_test = df_inputs_test.copy()
    #age
    shap.dependence_plot(0,shap_values,df_knees_test,interaction_index=0,show=True)
    #bmi
    shap.dependence_plot(17,shap_values,df_knees_test,interaction_index=17,show=True)
    #age by BMI
    shap.dependence_plot(0,shap_values,df_knees_test,interaction_index=17,show=True)
    #BMI by age
    shap.dependence_plot(17,shap_values,df_knees_test,interaction_index=0,show=True)
